...
vec4 lambda(vec2 z) {
    float EscapeVel2 = uEscapeVelocity*2.0;
    ...
    for (int i = 0; i < uIterations; i++) {
        ...
        vec2 b = vec2(sx * chy, cx * shy);

        z = vec2(
            uLambda.x * b.x - uLambda.y * b.y,
            uLambda.x * b.y + uLambda.y * b.x
        );

        if (dot(z,z) > EscapeVel2) {
            pallette = getPalette(i, z);
            break;
        }
    }
    return pallette;
