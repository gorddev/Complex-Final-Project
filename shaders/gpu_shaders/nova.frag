...
vec2 complex_pow(vec2 z, float power) {
    ...
    float angle = theta * power;

    return rp * vec2(cos(angle), sin(angle));
}
...
vec4 nova(vec2 frag) {
    ...
    for(i = 0; i < uIterations; i++) {
        vec2 z2 = complex_sqr(z);
        // prevent division by zero
        float denom = dot(z2, z2);
        if (denom < 1e-6) break;
        vec2 numerator = complex_pow(z - vec2(1.0, 0.0), 3.0);
        vec2 denominator = vec2(3.0 * z2.x, 3.0 * z2.y);
        vec2 step = complex_div(numerator, denominator);
        z = z - step + c * 0.1;
