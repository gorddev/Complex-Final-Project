    ...
    namespace fractal {
        ...
        struct Uniform {
            ...
            constexpr Uniform(const char name[], const char shaderHandle[],
                const UniformType type, const UniformBounds bounds = {-100.f, 100.f}, const UniformData defaultData = {})
                : bounds(bounds), propertyName(name), shaderHandle(shaderHandle), type(type), data(defaultData) {}
        };
